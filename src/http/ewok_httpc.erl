...
test(Num) ->
	test("http://localhost:8080/", Num).

test(Url, Num) ->
	start_test(Url, 0, 0, Num, erlang:now()).

start_test(Url, Count, Err, Num, Now) when Count < Num ->
	case ewok_httpc:get(Url) of
	{ok, _, _} ->
		start_test(Url, Count + 1, Err, Num, Now);
	_ ->
		start_test(Url, Count + 1, Err + 1, Num, Now)
	end;
start_test(_Url, Count, Err, _, {A1, B1, C1}) ->
	{A2, B2, C2} = erlang:now(),
