...
-record(params, {verbose, node, command, args}).
...
start() ->
	...
	Node = ewok_misc:localnode(NodeName),
    FullCommand = init:get_plain_arguments(),
	Args = parse_args(FullCommand, #params{verbose = false, node = Node}),
    #params{verbose = Verbose, node = Node, command = Command, args = Args} = Args,
    Inform = 
		case Verbose of
		false  -> 
			fun(_Format, _Args1) -> 
	...
	{'EXIT', {function_clause, [{?MODULE, action, _} | _]}} ->
		error("invalid command '~s'", [lists:flatten(
			ewok_misc:intersperse(" ", [atom_to_list(Command) | Args]))]),
		usage();
	{error, Reason} ->
		error("~p", [Reason]),
		halt(2);
...
fmt_stderr(Format, Args) -> 
	ewok_misc:format_stderr(Format ++ "~n", Args).
...
print_badrpc_diagnostics(Node) ->
    ...
    NodeHost = ewok_misc:nodehost(Node),
	...
	{ok, [[Home]]} -> 
		fmt_stderr("- current node home dir: ~s", [Home]);
	Other -> 
		fmt_stderr("- no current node home dir: ~p", [Other])
    end,
    fmt_stderr("- current node cookie hash: ~s", [ewok_misc:cookie_hash()]),
    ok.

parse_args(["-n", NodeS | Args], Params) ->
    Node = 
		case lists:member($@, NodeS) of        
		true  -> 
			list_to_atom(NodeS);
		false -> 
			ewok_misc:localnode(list_to_atom(NodeS))
		end,
    parse_args(Args, Params#params{node = Node});
parse_args(["-v" | Args], Params) ->
    parse_args(Args, Params#params{verbose = true});
...
usage() ->
    io:format("Usage: ewokctl [-v] [-n <node>] <command> [<arg> ...]
...
action(list_queues, Node, Args, Inform) ->
    ...
    display_info_list(rpc_call(Node, ewok_amqqueue, info_all,
                               [VHostArg, ArgAtoms]),
                      ArgAtoms);
...
action(list_exchanges, Node, Args, Inform) ->
    ...
    display_info_list(rpc_call(Node, ewok_exchange, info_all,
                               [VHostArg, ArgAtoms]),
                      ArgAtoms);
...
action(list_bindings, Node, Args, Inform) ->
    ...
    display_info_list(
      [lists:zip(InfoKeys, tuple_to_list(X)) ||
          X <- rpc_call(Node, ewok_exchange, list_bindings, [VHostArg])], 
      InfoKeys),
    ok;
...
action(list_connections, Node, Args, Inform) ->
    ...
    ArgAtoms = list_replace(node, pid,
                            default_if_empty(Args, [user, peer_address,
                                                    peer_port, state])),
    display_info_list(rpc_call(Node, ewok_networking, connection_info_all,
                               [ArgAtoms]),
                      ArgAtoms);
...
action(set_permissions, Node, VHost, [Username, CPerm, WPerm, RPerm], Inform) ->
    ...
    call(Node, {ewok_access_control, set_permissions,
...
action(clear_permissions, Node, VHost, [Username], Inform) ->
    ...
    call(Node, {ewok_access_control, clear_permissions, [Username, VHost]});

action(list_permissions, Node, VHost, [], Inform) ->
    Inform("Listing permissions in vhost ~p", [VHost]),
    display_list(call(Node, {ewok_access_control, list_vhost_permissions,
                             [VHost]})).
...
display_info_list(Results, InfoItemKeys) when is_list(Results) ->
    lists:foreach(
		fun (Result) -> 
			display_row([format_info_item(X, Result) || X <- InfoItemKeys])
		end, 
		Results),
    ok;
...
display_row(Row) ->
    io:fwrite(lists:flatten(ewok_misc:intersperse("\t", Row))),
    io:nl().
...
format_info_item(Key, Items) ->
	...
	#resource{name = Name} ->
		escape(Name);
	Value when Key =:= address; Key =:= peer_address andalso is_tuple(Value) ->
...
display_list(L) when is_list(L) ->
    lists:foreach(
		fun (I) when is_binary(I) ->
			...
			(I) when is_tuple(I) ->
				display_row([escape(V) || V <- tuple_to_list(I)])
		end,
		lists:sort(L)),
    ok;
