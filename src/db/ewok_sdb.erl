...
% @deprecated API
-export([list_items/1, list_items/2, delete_item/2, 
	query_items/2, query_items/3, request_v1/2]).
...
%% Returns a list of all items of a domain - 100 at a time.
...
query_items(Domain, QueryExp, Options) when is_list(Options) ->     
	request_action(#sdb_request{
		action = <<"Query">>, 
		result = <<"ItemName">>,
		params = [{<<"DomainName">>, Domain}, {<<"QueryExpression">>, QueryExp}],
		options = Options,
		meta = [request_id, next_token, box_usage]
	}).
...
%% @deprecated Use signature v2
request_v1(Action, Params) ->
	{ok, AccessKey, SecretKey} = gen_server:call(?SERVER, credentials),
    Timestamp = ewok_util:timestamp(),
	AllParams = [
		{<<"AWSAccessKeyId">>, AccessKey},
	...
	StringToSign = 
		list_to_binary([<<Param/binary, Value/binary>> || {Param, Value} <- lists:sort(CompareV1, AllParams)]),
%	?TTY({string_to_sign, StringToSign}),
    Signature = 
		base64:encode(crypto:sha_mac(SecretKey, StringToSign)),
